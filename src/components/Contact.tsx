    ...
    return (
        <div className="flex items-center justify-center gap-y-2.5 w-full h-[89.7vh] lg:h-[89vh] bg-gray-800 flex-col">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="w-[75%] lg:w-[60%] mx-auto">
                <div className="mb-4">
                    <label htmlFor="name" className="block text-emerald-400 font-bold mb-2"> Name </label>
                    <input placeholder="Enter your name" type="text" id="name"
                        {...register("name", nameOptions)} className={`border rounded w-full py-2 px-3 text-[#fea55f] ${errors.name ? "border-red-500" : ""}`} />
                ...
                <div className="mb-4">
                    <label htmlFor="subject" className="block text-emerald-400 font-bold mb-2"> Reason to connect </label>
                    <input placeholder="Reason for contact" type="text" id="subject"
                        {...register("subject", subjectOptions)} className={`border rounded w-full py-2 px-3 text-[#fea55f] ${errors.subject ? "border-red-500" : ""}`} />
                    {errors.subject && <p className="text-red-500 text-sm mt-1">{errors.subject.message}</p>}
                </div>

                <div className="mb-6">
                    <label htmlFor="message" className="block text-emerald-400 font-bold mb-2"> Message </label>
                    <textarea placeholder="Type the message..." id="message"
                        {...register("message", messageOptions)} className={`border rounded w-full py-2 px-3 text-[#fea55f] h-32 ${errors.message ? "border-red-500" : ""}`} >
                    </textarea>
